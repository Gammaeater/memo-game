...
class Game extends Component {
  ...
  render() {
    ...
    let gameStatus = <div className='Game-status'>
                      <div>Turns: {this.state.turnNo - 1}</div>
                      <div>Founnded Pairs: {this.state.pairsFound}</div>
                    </div>;
    ...
    if (this.state.pairsFound === this.cardController.NUM_IMAGES) {
      ...
      gameStatus = <div className='Game-status'>
                    <div>End of Game.</div>
                    <div>Finish{this.state.turnNo-1} Turns.</div>
                    <div><button onClick={this.onPlayAgain}>Play Again?</button></div>
                    </div>;
    }
      ...
      <div className='Game'>
        ...
        <div className='container'>
          <div className='row'>
            ...
            <div className='col-3'>
              <p>Ranking:</p>
              {this.rank.map((play) => <li>Name: {play['name']} Turns:: {play['turns']} </li>)}
            </div>
          </div>
        </div>
      </div>
